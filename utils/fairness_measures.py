...
def get_sufficiency(df, target_col, prediction_col, positive_value, cols):
    """
    Args:
        df (DataFrame):Input data
        target_col (str): ground truth of what we want to predict
    ...
    """
    dic_sufficiency = defaultdict(list)
    ...
    for col in cols:
        ...
        for c in classes:
            # numerator = P ( Y = 1 ) \intersection P ( R = 1 ) * P ( A = a) )
            # denom = P ( R = 1 ) * P ( A = a)

            # for both numerator and denominator I am not considering the division by len(df)

            for target in target_classes:
                numerator = len(df[(df[prediction_col] == positive_value) & (df[target_col] == target) & (df[col] == c)])
                denom = len(df[(df[target_col] == target) & (df[col] == c)])
